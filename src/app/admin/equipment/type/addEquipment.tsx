...
function AddEquipment({ showModal, setShowModal, setDatas }: EquipmentProps) {
    ...
    const handleOnSubmit = (e: React.MouseEvent<HTMLButtonElement>) => {
        e.preventDefault();
        toast.promise(
            post(adminEquipmentType, { name: name, description: description })
                .then((res) => {
                    setDatas((prev: any) => [convertDataToEquipment(res.data.data), ...prev]);
                    setShowModal(false);
                }),
            {
                pending: "Đang xử lý...",
                success: "Thêm quân tư trang thành công",
                error: "Thêm quân tư trang thất bại",
            }
        ).catch((err) => {
        ...
        >
            <Box className='xl:w-[50%] lg:w-[70%] md:w-[90%] w-[95%] max-h-[90vh] bg-white rounded-xl shadow-2xl overflow-hidden'>
                <div className='bg-[var(--color-text)] text-white p-5 relative'>
                    <h2 className='text-2xl font-semibold text-center'>Thêm quân tư trang</h2>
                    <button 
