...
function convertDataToPractice(data: any): FitnessTest {
    return {
        id: data.id,
        name: data.name,
        unit: data.unit,
        higherIsBetter: data.higher_is_better ? "Có" : "Không",
        createdAt: new Date(data.created_at),
        updatedAt: new Date(data.updated_at),
        thresholdsExcellentThreshold: data.thresholds.excellent_threshold,
        thresholdsGoodThreshold: data.thresholds.good_threshold,
        thresholdsPassThreshold: data.thresholds.pass_threshold,
    }
}
...
function AddPractice({
    ...
    const [excellentThreshold, setExcellentThreshold] = useState<string>('');
    const [goodThreshold, setGoodThreshold] = useState<string>('');
    ...
    const handleOnSubmit = (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        toast.promise(
            post(adminFitnessTest, { 
                ...
                .then((res) => {
                    setDatas((prev: any) => [convertDataToPractice(res.data.data), ...prev]);
                    setShowModal(false);
                }),
            {
                pending: "Đang xử lý...",
                success: "Tạo bài kiểm tra thể lực thành công",
                error: "Tạo bài kiểm tra thể lực thất bại",
            }
        ).catch((err) => {
        ...
        >
            <Box className='xl:w-[50%] lg:w-[70%] md:w-[90%] w-[95%]  bg-white rounded-xl shadow-2xl overflow-hidden'>
                <div className='bg-[var(--color-text)] text-white p-5 relative'>
                    <h2 className='text-2xl font-semibold text-center'>Thêm bài kiểm tra thể lực</h2>
                    <button 
