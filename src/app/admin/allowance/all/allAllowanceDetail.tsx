...
interface AdminAllowanceDetailProps {
    readonly allowanceDetail: AllowanceStudent | undefined;
    readonly showModal: boolean;
    readonly setShowModal: React.Dispatch<React.SetStateAction<boolean>>;
}

function AllAllowanceDetail({ allowanceDetail, showModal, setShowModal }: AdminAllowanceDetailProps) {
    const [statusColor, setStatusColor] = useState<string>('text-yellow-500');
                ...
                <div className="p-6">
                    ...
                    <div className='w-full flex flex-col items-center justify-center mb-6'>
                        <h1 className='text-xl md:text-2xl font-bold text-[color:var(--color-text)]'>
                            {allowanceDetail?.studentName ?? "Không tìm thấy thông tin"}
                        </h1>
                        <p className='text-gray-500'>{allowanceDetail?.studentEmail}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 px-2 md:px-4">
                        <InfoItem
                            icon={faEnvelope}
                            label="Email"
                            value={allowanceDetail?.studentEmail ?? 'N/A'}
                        />
